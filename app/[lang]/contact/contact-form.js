    ...
    <form className={`${cardBgCls} max-w-lg mx-auto mt-20 px-5 py-8`}>
      ...
      {content.inputs.map((input) => (
        <div className={inputFieldCls}>
          ...
          <label for={input.props.id} className={inputLabelCls}>
            {input.label[lang]}
          </label>
        </div>
      ))}
      ...
      <div className={inputFieldCls}>
        <label for="subject" className="block mb-2">
          {content.subjects.label[lang]}
        </label>
        <select name="subject" id="subject" className={`block w-full p-1 bg-transparent ${underline}`}>
          {content.subjects.options.map((option) => (
            <option value={option.en}>{option[lang]}</option>
          ))}
        </select>
      </div>
      ...
      <div className={inputFieldCls}>
        ...
        <label for="message" className={inputLabelCls}>
